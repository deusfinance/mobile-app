...
class XDaiStockData extends SyncData {
  @override
  Future<Map<String, StockPrice>> getPrices() async {
    final response = await http.get(BackendEndpoints.PRICE_JSON_XDAI_1);
  ...
  Future<bool> getStockAddresses() async {
    ...
    final response = await http
        .get(Uri.parse("https://oracle1.deus.finance/xdai/conducted.json"));
    if (response.statusCode == 200) {
  ...
  Future<Map<String, ContractInputData>?> getInfoOracle1() async {
    final response = await http.get(BackendEndpoints.SIGNATURES_JSON_XDAI_1);
    if (response.statusCode == 200) {
  ...
  Future<Map<String, ContractInputData>?> getInfoOracle2() async {
    final response = await http.get(BackendEndpoints.SIGNATURES_JSON_XDAI_2);
    if (response.statusCode == 200) {
      final Map<String, ContractInputData> contractInputData = new Map();
      final Map<String, dynamic> map = json.decode(response.body);
      map.forEach((key, value) {
  ...
  Future<Map<String, ContractInputData>?> getInfoOracle3() async {
    final response = await http.get(BackendEndpoints.SIGNATURES_JSON_XDAI_3);
    if (response.statusCode == 200) {
      final Map<String, ContractInputData> contractInputData = new Map();
      final Map<String, dynamic> map = json.decode(response.body);
      map.forEach((key, value) {
