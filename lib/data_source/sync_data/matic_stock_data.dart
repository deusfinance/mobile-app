...
class MaticStockData extends SyncData {
  @override
  Future<Map<String, StockPrice>> getPrices() async {
    var response = await http.get(BackendEndpoints.PRICE_JSON_MATIC_1);
  ...
  Future<bool> getStockAddresses() async {
    ...
    var response = await http.get(Uri.parse("https://oracle1.deus.finance/heco/conducted.json"));
    if (response.statusCode == 200) {
  ...
  Future<Map<String, ContractInputData>?> getInfoOracle1() async {
    var response = await http.get(BackendEndpoints.SIGNATURES_JSON_MATIC_1);
    if (response.statusCode == 200) {
  ...
  Future<Map<String, ContractInputData>?> getInfoOracle2() async {
    var response = await http.get(BackendEndpoints.SIGNATURES_JSON_MATIC_2);
    if (response.statusCode == 200) {
      Map<String, ContractInputData> contractInputData = new Map();
      var map = json.decode(response.body);
      map.forEach((key, value) {
  ...
  Future<Map<String, ContractInputData>?> getInfoOracle3() async {
    var response = await http.get(BackendEndpoints.SIGNATURES_JSON_MATIC_3);
    if (response.statusCode == 200) {
      Map<String, ContractInputData> contractInputData = new Map();
      var map = json.decode(response.body);
      map.forEach((key, value) {
