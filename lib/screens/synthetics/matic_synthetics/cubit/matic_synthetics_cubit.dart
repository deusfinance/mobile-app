...
import 'matic_synthetics_state.dart';

class MaticSyntheticsCubit extends Cubit<MaticSyntheticsState> {
  MaticSyntheticsCubit() : super(MaticSyntheticsInitialState());

  init() async {
    emit(MaticSyntheticsLoadingState(state));

    bool res1 = await MaticStockData.getData();
    state.prices = await MaticStockData.getPrices();
    if (res1 && state.prices != null) {
      ...
      state.inputController.stream
        ...
        if (state is MaticSyntheticsSelectAssetState) {
          ...
          emit(MaticSyntheticsAssetSelectedState(state, isInProgress: true));
          ...
          emit(MaticSyntheticsAssetSelectedState(state, isInProgress: false));
        }
      });
      state.timer =
          Timer.periodic(Duration(seconds: 14), (Timer t) => getPrices());
      emit(MaticSyntheticsSelectAssetState(state));
    } else {
      emit(MaticSyntheticsErrorState(state));
    }
  }
  ...
  Future getAllowances() async {
    emit(MaticSyntheticsAssetSelectedState(state,
  ...
  Future<String> getTokenAddress(Token token) async {
    String tokenAddress;
    if (token.getTokenName() == "usdc") {
      tokenAddress = state.service.usdc;
    } else {
      StockAddress stockAddress = MaticStockData.getStockAddress(token);
    ...
    if (state is MaticSyntheticsSelectAssetState) {
      ...
      if (isBuy()) {
        if ((state.fromToken as CryptoCurrency).getAllowances() >=
          ...
          emit(MaticSyntheticsAssetSelectedState(state, approved: true));
        } else {
          state.inputController.add(input);
          emit(MaticSyntheticsAssetSelectedState(state, approved: false));
        }
      } else {
  ...
  reversePriceRatio() {
    if (state is MaticSyntheticsAssetSelectedState) {
      emit(MaticSyntheticsAssetSelectedState(state,
  ...
  Future<void> setMode(Mode mode) async {
    if (!state.isInProgress && state.toToken != null) {
      ...
      if (checkMarketClosed(isBuy() ? state.toToken : state.fromToken, mode)) {
        state.marketClosed = true;
        emit(MaticSyntheticsAssetSelectedState(state, mode: mode));
      } else {
        state.marketClosed = false;
        emit(MaticSyntheticsAssetSelectedState(state, mode: mode));
        await getAllowances();
  ...
  void closeToast() {
    if (state is MaticSyntheticsTransactionPendingState)
      emit(MaticSyntheticsTransactionPendingState(state, showingToast: false));
    else if (state is MaticSyntheticsTransactionFinishedState)
      emit(MaticSyntheticsTransactionFinishedState(state, showingToast: false));
  }
  ...
  Future approve(Gas gas) async {
    if (!state.isInProgress) {
      try {
        ...
        emit(MaticSyntheticsTransactionPendingState(state,
        ...
        result.listen((event) {
          ...
          if (event.status) {
            state.approved = true;
            emit(MaticSyntheticsTransactionFinishedState(state,
                transactionStatus: TransactionStatus(
                    "Approve ${state.fromToken.name}",
                    Status.SUCCESSFUL,
    ...
    if (state.approved && !state.isInProgress) {
      if (gas != null) {
        ...
        List<ContractInputData> oracles =
            await MaticStockData.getContractInputData(tokenAddress,
  ...
  makeBuyTransaction() async {
    String tokenAddress = await getTokenAddress(state.toToken);
    List<ContractInputData> oracles = await MaticStockData.getContractInputData(
        tokenAddress,
        await state.service.ethService.ethClient.getBlockNumber());
    if (oracles.length >= 2) {
      //sort oracles on price and then on oracle number
