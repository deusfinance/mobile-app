...
import 'mainnet_synthetics_state.dart';

class MainnetSyntheticsCubit extends Cubit<MainnetSyntheticsState> {
  MainnetSyntheticsCubit() : super(MainnetSyntheticsInitialState());

  init() async {
    emit(MainnetSyntheticsLoadingState(state));

    bool res1 = await StockData.getData();
    state.prices = await StockData.getPrices();
    if (res1 && state.prices != null) {
      ...
      state.inputController.stream
          ...
          .listen((s) {
        if (state is MainnetSyntheticsSelectAssetState) {} else {
          emit(MainnetSyntheticsAssetSelectedState(state, isInProgress: true));
          ...
          emit(MainnetSyntheticsAssetSelectedState(state, isInProgress: false));
        }
      });
      state.timer =
          Timer.periodic(Duration(seconds: 14), (Timer t) => getPrices());
      emit(MainnetSyntheticsSelectAssetState(state));
    } else {
      emit(MainnetSyntheticsErrorState(state));
    }
  }
  ...
  Future getAllowances() async {
    emit(MainnetSyntheticsAssetSelectedState(state,
  ...
  fromTokenChanged(Token selectedToken) async {
    state.toToken = CurrencyData.dai;
  ...
  toTokenChanged(Token selectedToken) async {
    state.fromToken = CurrencyData.dai;
    ...
    if (state is MainnetSyntheticsSelectAssetState) {} else {
      ...
      if (isBuy()) {
        if ((state.fromToken as CryptoCurrency).getAllowances() >=
          ...
          emit(MainnetSyntheticsAssetSelectedState(state, approved: true));
        } else {
          state.inputController.add(input);
          emit(MainnetSyntheticsAssetSelectedState(state, approved: false));
        }
      } else {
  ...
  reversePriceRatio() {
    if (state is MainnetSyntheticsAssetSelectedState) {
      emit(MainnetSyntheticsAssetSelectedState(state,
  ...
  Future<void> setMode(Mode mode) async {
    if (!state.isInProgress && state.toToken != null) {
      ...
      if (checkMarketClosed(isBuy() ? state.toToken : state.fromToken, mode)) {
        state.marketClosed = true;
        emit(MainnetSyntheticsAssetSelectedState(state, mode: mode));
      } else {
        state.marketClosed = false;
        emit(MainnetSyntheticsAssetSelectedState(state, mode: mode));
        await getAllowances();
  ...
  void closeToast() {
    if (state is MainnetSyntheticsTransactionPendingState)
      emit(
          MainnetSyntheticsTransactionPendingState(state, showingToast: false));
    else if (state is MainnetSyntheticsTransactionFinishedState)
      emit(MainnetSyntheticsTransactionFinishedState(state,
          showingToast: false));
  }
  ...
  Future sell(Gas gas) async {
    if (state.approved && !state.isInProgress) {
      if (gas != null) {
        ...
        List<ContractInputData> oracles = await StockData.getContractInputData(
            tokenAddress,
