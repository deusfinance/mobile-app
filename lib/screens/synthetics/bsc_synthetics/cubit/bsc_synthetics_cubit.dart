...
import 'bsc_synthetics_state.dart';

class BscSyntheticsCubit extends Cubit<BscSyntheticsState> {
  BscSyntheticsCubit() : super(BscSyntheticsInitialState());

  init() async {
    emit(BscSyntheticsLoadingState(state));

    bool res1 = await BscStockData.getData();
    state.prices = await BscStockData.getPrices();
    if (res1 && state.prices != null) {
      ...
      state.inputController.stream
          ...
          .listen((s) {
        if (state is BscSyntheticsSelectAssetState) {} else {
          emit(BscSyntheticsAssetSelectedState(state, isInProgress: true));
          ...
          emit(BscSyntheticsAssetSelectedState(state, isInProgress: false));
        }
      });
      state.timer =
          Timer.periodic(Duration(seconds: 14), (Timer t) => getPrices());
      emit(BscSyntheticsSelectAssetState(state));
    } else {
      emit(BscSyntheticsErrorState(state));
    }
  }
    ...
    if (state is BscSyntheticsSelectAssetState) {} else {
      ...
      if (isBuy()) {
        if ((state.fromToken as CryptoCurrency).getAllowances() >=
          ...
          emit(BscSyntheticsAssetSelectedState(state, approved: true));
        } else {
          state.inputController.add(input);
          emit(BscSyntheticsAssetSelectedState(state, approved: false));
        }
      } else {
  ...
  reversePriceRatio() {
    if (state is BscSyntheticsAssetSelectedState) {
      emit(BscSyntheticsAssetSelectedState(state,
  ...
  Future<void> setMode(Mode mode) async {
    if (!state.isInProgress && state.toToken != null) {
      ...
      if (checkMarketClosed(isBuy() ? state.toToken : state.fromToken, mode)) {
        state.marketClosed = true;
        emit(BscSyntheticsAssetSelectedState(state, mode: mode));
      } else {
        state.marketClosed = false;
        emit(BscSyntheticsAssetSelectedState(state, mode: mode));
        await getAllowances();
  ...
  void closeToast() {
    if (state is BscSyntheticsTransactionPendingState)
      emit(
          BscSyntheticsTransactionPendingState(state, showingToast: false));
    else if (state is BscSyntheticsTransactionFinishedState)
      emit(BscSyntheticsTransactionFinishedState(state,
          showingToast: false));
  }
  ...
  Future approve(Gas gas) async {
    if (!state.isInProgress) {
      try {
        ...
        emit(BscSyntheticsTransactionPendingState(state,
  ...
  Future sell(Gas gas) async {
    if (state.approved && !state.isInProgress) {
      if (gas != null) {
        ...
        List<ContractInputData> oracles = await BscStockData.getContractInputData(
            tokenAddress,
