...
class XDaiSyntheticsCubit extends Cubit<XDaiSyntheticsState> {
  ...
  void closeToast() {
    if (state is XDaiSyntheticsTransactionPendingState)
      emit(
          XDaiSyntheticsTransactionPendingState(state, showingToast: false));
    else if (state is XDaiSyntheticsTransactionFinishedState)
      emit(XDaiSyntheticsTransactionFinishedState(state,
          showingToast: false));
  }
  ...
  Future sell(Gas gas) async {
    if (state.approved && !state.isInProgress) {
      if (gas != null) {
        ...
        List<ContractInputData> oracles = await XDaiStockData.getContractInputData(
            tokenAddress,
        ...
        if (oracles.length >= 2) {
          try {
            ...
            var res = await state.service.buy(
                tokenAddress, state.toValue.toStringAsFixed(18), inputOracles,maxPrice, gas);
            emit(XDaiSyntheticsTransactionPendingState(state,
                transactionStatus: TransactionStatus(
                    "Buy ${state.toFieldController.text} ${state.toToken
                        .getTokenName()}",
  ...
  makeBuyTransaction() async {
    ...
    if (oracles.length >= 2) {
      ...
      Transaction transaction = await state.service.makeBuyTransaction(
          tokenAddress, state.toValue.toStringAsFixed(18), inputOracles, maxPrice);
