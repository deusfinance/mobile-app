...
class _SendAssetScreenState extends State<SendAssetScreen> {
                      ...
                      children: [
                        Padding(
                          ...
                          child: state.walletAsset.logoPath != null && state.walletAsset.logoPath != ""
                              ? state.walletAsset.logoPath!.showCircleImage(radius: 25)
                          // CircleAvatar(
                          //         radius: 25,
                  ...
                  Visibility(
                    ...
                    child: Padding(
                      ...
                      child: Text(
                        ...
                        style: TextStyle(
                            fontSize: 12,
                            color: state.addressConfirmed ? Colors.green : Colors.red),
                      ),
                    ),
                  ),
                  SizedBox(
                      ...
                      child: SelectionButton(
                        ...
                        onPressed: (bool selected) async {
                          switch (state.walletService.chain.id) {
                            case 100:
                            // return "xDai";
                              Transaction? transaction = await context.read<SendAssetCubit>().makeTransferTransaction();
                              WidgetsBinding.instance!.focusManager.primaryFocus?.unfocus();
                              if (transaction != null) {
                                Gas? gas = await showConfirmGasFeeDialog(transaction, Network.XDAI);
                                await context.read<SendAssetCubit>().transfer(gas);
                              }
                              break;
                          // case 1:
                          //   return "ETH";
                          case 56:
                            ...
                            WidgetsBinding.instance!.focusManager.primaryFocus?.unfocus();
                            if (transaction != null) {
                              Gas? gas = await showConfirmGasFeeDialog(transaction, Network.BSC);
                              await context.read<SendAssetCubit>().transfer(gas);
                            }
                            break;
                          // case 128:
  ...
  Widget _buildToastWidget(SendAssetState state) {
    if (state is TransactionPendingState && state.showingToast) {
      return Align(alignment: Alignment.bottomCenter, child: _buildTransactionPending(state, state.transactionStatus));
      ...
      if (state.transactionStatus.status == Status.PENDING) {
        ...
        return Align(alignment: Alignment.bottomCenter, child: _buildTransactionSuccessFul(state, state.transactionStatus));
      } else if (state.transactionStatus.status == Status.FAILED) {
        return Align(alignment: Alignment.bottomCenter, child: _buildTransactionFailed(state, state.transactionStatus));
      }
    }
    return Container();
  }
  ...
  Widget _buildTransactionPending(SendAssetState state, TransactionStatus transactionStatus) {
    return Container(
      ...
      child: Toast(
        ...
        onPressed: () {
          if (transactionStatus.hash != "") {
            _launchInBrowser(transactionStatus.transactionUrlWithChain(state.walletService.chain));
          }
        },
        onClosed: () {
  ...
  Widget _buildTransactionSuccessFul(SendAssetState state, TransactionStatus transactionStatus) {
    return Container(
      margin: EdgeInsets.all(8),
      child: Toast(
        label: transactionStatus.label,
  ...
  Widget _buildTransactionFailed(SendAssetState state, TransactionStatus transactionStatus) {
    return Container(
      margin: EdgeInsets.all(8),
      child: Toast(
        label: transactionStatus.label,
