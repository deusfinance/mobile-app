...
class BakktService {
  ...
  Future<String> getAllowances(token) async {
    ...
    final res = await ethService.query(tokenContract, "allowance", [await address, ethService.getAddrHex("bakkt_swap_contract")]);
  ...
  Future<String> approve(token) async {
    ...
    DeployedContract tokenContract = await ethService.loadTokenContract(token);
    var res = await ethService.submit(await credentials, tokenContract, "approve", [await ethService.getAddrHex("bakkt_swap_contract"), getWei(amount, token)]);
    return res;
  }
  ...
  Future<String> swapTokens(fromToken, toToken, tokenAmount) async{
    ...
    DeployedContract contract  = await ethService.loadContractWithGivenAddress("bakkt_swap_contract", await ethService.getAddrHex("bakkt_swap_contract"));
...
//      return await ethService.submit(await credentials, contract, "buy", [getWei(BigInt.from(0.95 * bakktAmount), toToken), getWei(tokenAmount, toToken)]);
  ...
  Future<String> getAmountsOut(fromToken, toToken, amountIn) async{
    DeployedContract contract = await ethService.loadContractWithGivenAddress("sps", await ethService.getAddrHex("bakkt_swap_contract"));
    if(fromToken == "bakkt" && toToken == "deus"){
  ...
  Future<String> approveStocks() async {
    ...
    var res = await ethService.submit(await credentials, tokenContract, "approve", [await ethService.getAddrHex("stocks_contract"), getWei(amount, "ether")]);
    return res;
  }
  ...
  Future<String> getAllowancesStocks() async {
    ...
    DeployedContract tokenContract = await ethService.loadTokenContract("dai");
    final res = await ethService.query(tokenContract, "allowance", [await address, ethService.getAddrHex("stocks_contract")]);
  ...
  Future<String> buyStock(stockAddress, amount, ContractInputData info) async {
    ...
    await ethService.loadContract("stocks_contract");
