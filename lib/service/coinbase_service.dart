...
class CoinBaseService {
  ...
  Future<String> getAllowances(tokenName) async {
    ...
    final res = await ethService.query(tokenContract, "allowance", [await address, ethService.getAddrHex("coinbase_swap_contract")]);
  ...
  Future<String> approve(tokenName) async {
    ...
    DeployedContract tokenContract = await ethService.loadTokenContract(tokenName);
    var res = await ethService.submit(await credentials, tokenContract, "approve", [ethService.getAddrHex("coinbase_swap_contract"), getWei(amount, tokenName)]);
    return res;
  }
  ...
  Future<String> swapTokens(fromToken, toToken, tokenAmount) async{
    ...
    DeployedContract contract = await ethService.loadContractWithGivenAddress("bakkt_swap_contract", await ethService.getAddrHex("coinbase_swap_contract"));
...
//      return await ethService.submit(await credentials, contract, "sell", [getWei(tokenAmount, fromToken), getWei(BigInt.from(0.95 * deusAmount), toToken)]);
      ...
      var coinBaseAmount = fromWei(res[0], toToken);
      //      TODO
//      return await ethService.submit(await credentials, contract, "buy", [getWei(BigInt.from(0.95 * coinBaseAmount), toToken), getWei(tokenAmount, toToken)]);
  ...
  Future<String?> getAmountsOut(fromToken, toToken, amountIn) async{
    ...
    if(fromToken == "coinbase" && toToken == "deus"){
      ...
      var res = await ethService.query(bakktContract, "calculatePurchaseReturn", [getWei(amountIn, fromToken)]);
      return fromWei(res[0], toToken);
    }
  }
